...
# Set timescaledb database
echo
echo "# Setting up timescaledb environment ... ###########################"
if [ -z ${TIMESCALEDBNAME+x} ]; then
  export TIMESCALEDBNAME="timescaledb";
  echo "NOTE:"
  echo "   TIMESCALEDBNAME has not been set. Using default TIMESACLEDBNAME=${TIMESCALEDBNAME}."
  echo "   To change the default TIMESCALEDBNAME, use the docker run \"-e\" switch."
  echo "   Example: \"docker run -e \"TIMESCALEDBNAME=myCustomDatabaseName\" tumgis/3dcitydb-postgis\""
fi
...
echo
echo "Creating database $TIMESCALEDBNAME ..."
echo "CREATE DATABASE \"$TIMESCALEDBNAME\";" | "${psql[@]}"
echo "Creating database $TIMESCALEDBNAME ...done!"
...
# Setup Timescale extension
echo
timescaledb-tune --quiet --yes --dry-run >> /var/lib/postgresql/data/postgresql.conf
echo
echo "Create Timescale extensions in database $TIMESCALEDBNAME ..."
"${psql[@]}" -d "$TIMESCALEDBNAME" <<-EOSQL
    CREATE EXTENSION IF NOT EXISTS timescaledb CASCADE;
EOSQL
echo "Setting up Timescale extension in database $TIMESCALEDBNAME ...done!"
